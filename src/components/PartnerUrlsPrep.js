...
class PartnerUrlsPrep extends Component {
  ...
  render() {
    let partnerUrlsPrep = this.state.urlsprep.map(
        (partnerUrl,idx) => { return(<PartnerUrlPrep key={idx} urlprep={partnerUrl} showEditBtns={this.state.editMode} showDeleteBtns={this.state.deleteMode} onUrlChange={ this.onUpdateUrl } onDeleteUrl={ this.onDeleteUrl }/>); }
    );
